"""
Theoretical distributions and curves for single-channel currents.

Every function here takes a mechanism and *returns arrays* -- ``t, f`` pairs
ready to plot -- rather than drawing anything. Only :func:`png_save_pdf_fig`
touches matplotlib, and it is a convenience for saving a figure the caller
could just as well build itself.

This module was called ``scplotlib`` for fifteen years, which made it read as a
duplicate of EKDIST's ``ekplot`` -- an empirical histogram module with no
overlap whatsoever. ``scalcs.scplotlib`` still imports and re-exports
everything here, with a DeprecationWarning.
"""
